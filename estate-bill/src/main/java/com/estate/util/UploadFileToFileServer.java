...
public class UploadFileToFileServer {
    ...
    //登录文件服务器的地址
    private  String loginUrl = "http://localhost:8088/upload/login";
    //文件服务器的删除地址
    private  String delUrl = "http://localhost:8088/upload/fileDel";
    //接收文件的服务器地址
    private  String serverUrl = "http://localhost:8088/upload/fileUpload";
    ...
    public  String delFile(String path) throws Exception{
        String resultSet = null;
        try{
            HttpURLConnection huc = (HttpURLConnection)  new URL(delUrl).openConnection();
            //指定HTTP内容类型及URL格式为form表单格式
            //huc.setRequestProperty("Content-Type","application/x-www-form-urlencoded");
            // 设置允许output
            huc.setDoOutput(true);
            // 设置提交方式为post方式
            huc.setRequestMethod("POST");
            String parameter="path="+path.substring(path.indexOf("name=")+5);
            OutputStream os = huc.getOutputStream();
            os.write(parameter.getBytes("utf-8"));//指定URL及参数编码格式
            os.flush();
            os.close();
     ...
     * @throws Exception
     */
    public String fileUpload(byte[] bytes,String filename) throws Exception{
        String resultSet = null;
        try{
